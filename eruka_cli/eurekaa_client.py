...
class EurekaaClient:
    ...
    def search_books(self, query: str, limit: int) -> list[dict[str, Any]]:
        errors: list[str] = []
        try:
            books = self._search_eurekaa_books(query, limit)
            if books:
                return books
        except Exception as exc:
            errors.append(f"Eurekaa GraphQL: {exc}")

        try:
            books = self._search_google_books(query, limit)
            if books:
                return books
        except Exception as exc:
        ...
        try:
            books = self._search_open_library(query, limit)
            if books:
                return books
        except Exception as exc:
    ...
    def doctor(self, from_chrome: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            checks["public_courses_summary"] = {"ok": bool(summary.get("courses")), "courses": summary.get("courses")}
        except Exception as exc:
        ...
        try:
            books = self.search_books("digital marketing", 1)
            checks["book_search"] = {"ok": bool(books), "count": len(books)}
        except Exception as exc:
            checks["book_search"] = {"ok": False, "error": str(exc)}

        try:
            auth = resolve_auth(from_chrome=from_chrome)
            checks["auth"] = {"ok": auth.present, "source": auth.source, "email": auth.email, "token": auth.masked()}
        except Exception as exc:
